...
打开 <http://127.0.0.1:8000/>。如果需要临时局域网访问，必须同时将 `--host` 改为 `0.0.0.0`，并用 `WPG_ALLOWED_CLIENT_CIDRS=127.0.0.0/8,::1/128,<真实-LAN-CIDR>` 显式限定 direct peer；正式对外使用应启用反向代理、访问控制和 HTTPS。
