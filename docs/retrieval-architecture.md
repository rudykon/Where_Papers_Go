...
2. LLM 将模糊表达生成受约束的受控主题、中英文扩展词和 Search API 查询。
...
7. 用 IDF、字段权重、受控主题、图路径、两路向量、LightRAG 信号和稿件边界融合，最后由 LLM 在已知候选 ID 内受约束重排。
...
- 74/74 自动测试通过，其中包含真实 LightRAG 本地存储导入、`mix` 查询、Web API 配置校验和 Search API 引用安全校验。
